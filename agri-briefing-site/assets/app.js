...
function renderFlowerWatch(items){
  ...
  const section=document.getElementById('flowerWatchSection'), target=document.getElementById('flowerWatch');
  if(!section||!target)return;
  section.hidden=false;
  target.innerHTML=items.map(x=>`<article class="flower-watch-card"><small>${esc(x.flower)}</small><h3>${esc(x.variety||'품종 동향')}</h3><strong>${esc(x.signal)}</strong><p>${esc(x.evidence)}</p><a href="${esc(x.sourceUrl)}" target="_blank" rel="noopener noreferrer">근거 보기 · ${esc(x.sourceName)}</a></article>`).join('');
}
async function loadBriefing(){
  ...
  for(const id of ['briefingDate','footerDate']){const el=document.getElementById(id);if(el)el.textContent=d.date}
  for(const [id,value] of [['briefingSummary',d.summary],['signal',d.signal]]){const el=document.getElementById(id);if(el)el.textContent=value}
  renderFlowerWatch(d.flowerWatch);const stories=document.getElementById('stories');if(!stories)throw Error('브리핑 화면을 준비하지 못했습니다.');stories.innerHTML=d.stories.map(storyCard).join('');observeStories();
}
...
async function loadArchive(){try{const base=pathBase;const r=await fetch(`${base}data/archive.json`,{cache:'no-store'});const xs=await r.json();const q=document.getElementById('archiveList');if(q)q.innerHTML=xs.map(x=>`<a class="archive-item" href="${base}briefing/${esc(x.date)}/"><strong>${esc(x.title)}</strong><span>${esc(x.date)} · ${esc(x.signal)}</span></a>`).join('');if(document.querySelector('main')?.dataset.archivePage==='true')renderArchivePage(xs,base)}catch(e){console.warn('Archive unavailable',e)}}
function renderArchivePage(items,base){const stories=document.getElementById('stories');if(!stories)return;stories.innerHTML=`<section class="archive-page"><div class="eyebrow">BROWSE</div><h2>전체 브리핑</h2><input id="archiveSearch" class="archive-search" placeholder="제목·시그널 검색"><div id="archiveResults"></div></section>`;const out=document.getElementById('archiveResults');if(!out)return;const paint=filter=>out.innerHTML=items.filter(x=>JSON.stringify(x).toLowerCase().includes(filter.toLowerCase())).map(x=>`<a class="archive-result" href="${base}briefing/${x.date}/"><small>${esc(x.date)}</small><h3>${esc(x.title)}</h3><p>${esc(x.signal)}</p><span>${x.headlines.map(esc).join(' · ')}</span></a>`).join('')||'<p>검색 결과가 없습니다.</p>';paint('');const search=document.getElementById('archiveSearch');if(search)search.oninput=e=>paint(e.target.value)}
const drawer=document.getElementById('archive'),overlay=document.getElementById('overlay'),archiveButton=document.getElementById('archiveBtn'),closeArchive=document.getElementById('closeArchive');if(drawer&&overlay&&archiveButton&&closeArchive){archiveButton.onclick=()=>{drawer.classList.add('open');overlay.classList.add('open')};closeArchive.onclick=overlay.onclick=()=>{drawer.classList.remove('open');overlay.classList.remove('open')}};
