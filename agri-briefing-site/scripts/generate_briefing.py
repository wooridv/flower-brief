...
from briefing_lib import enrich_story_images, is_duplicate, is_korean_nonworking_day, validate_briefing

ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data"
KST = ZoneInfo("Asia/Seoul")
FIELDS = ["title", "shortTitle", "date", "country", "category", "sourceName", "sourceUrl", "imageUrl", "summary", "whyItMatters", "background", "technology", "marketOutlook", "keywords"]
...
FLOWER_WATCH_SCHEMA = {"type": "object", "additionalProperties": False, "required": ["flower", "variety", "signal", "evidence", "sourceName", "sourceUrl"], "properties": {field: {"type": "string"} for field in ["flower", "variety", "signal", "evidence", "sourceName", "sourceUrl"]}}
SCHEMA = {"type": "object", "additionalProperties": False, "required": ["date", "summary", "signal", "mostImportantChange", "attentionPoints", "flowerWatch", "stories"], "properties": {"date": {"type": "string"}, "summary": {"type": "string"}, "signal": {"type": "string"}, "mostImportantChange": {"type": "string"}, "attentionPoints": {"type": "array", "items": {"type": "string"}}, "flowerWatch": {"type": "array", "items": FLOWER_WATCH_SCHEMA}, "stories": {"type": "array", "items": STORY_SCHEMA}}}
...
절대 URL, 출처, 날짜, 수치를 만들어내지 말고 검색 결과에서 확인된 사실만 쓴다. sourceUrl은 실제 원문 URL이어야 한다. imageUrl은 가능하면 원문의 OG 이미지나 공식 발표 이미지 URL을 넣고, 확인하지 못하면 빈 문자열로 둔다. 아래 최근 소개와 URL·제목·동일 사건이 중복되는 기사는 제외하라. 후속 보도만 허용하며 그 사실을 background에 밝혀라. 모든 분석은 한국어로, 독자가 뉴스를 이해하는 데 필요한 동향과 의미만 설명한다. 행동 지시, 실행 과제, 사업 제안은 쓰지 않는다. 근거 없는 시장 수치나 예측은 금지한다.\n최근 소개: {json.dumps(prior, ensure_ascii=False)}'''
...
def generate(client: OpenAI, today: str, history: list[dict]) -> dict:
    ...
    for attempt in range(2):
        try:
            ...
            print(f"Publisher card images found: {images_added}/{len(payload['stories'])}")
            return payload
        except Exception as exc:
